...
const ContactPage = () => {
    ...
    const showSuccess = (message) => {
        toast.current.show({
            ...
            style: { background: 'linear-gradient(135deg, #137fec 0%, #10b981 100%)', color: '#fff', borderRadius: '12px', border: 'none' },
        ...
        <div className="min-h-screen bg-[#f6f7f8] dark:bg-[#101922] text-slate-900 dark:text-white font-sans flex flex-col">
            ...
            <header className="sticky top-0 z-50 w-full border-b border-[#e7edf3] dark:border-slate-800 bg-white/80 dark:bg-[#101922]/90 backdrop-blur-md">
                <div className="px-4 md:px-10 lg:px-40 py-3 flex items-center justify-between">
                    ...
                    <button onClick={handleGoBack} className="flex items-center gap-2 text-slate-500 hover:text-[#137fec] transition-colors">
                ...
                <section className="relative py-16 px-6 md:px-12 bg-white dark:bg-[#101922] border-b border-slate-100 dark:border-slate-800">
                    <div className="max-w-[900px] mx-auto text-center relative z-10">
                        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#137fec]/10 text-[#137fec] mb-6">
                            ...
                            <span className="text-sm font-bold uppercase tracking-wider">We're Here For You</span>
                        </div>
                        <h1 className="text-4xl md:text-5xl lg:text-6xl font-black leading-tight text-slate-900 dark:text-white mb-6">
                            Contact <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#137fec] to-blue-400">Us</span>
                        </h1>
                ...
                <section className="py-12 px-6 md:px-12 bg-[#f6f7f8] dark:bg-[#0d141b]">
                    <div className="max-w-[1100px] mx-auto">
                        <div className="grid md:grid-cols-3 gap-6 mb-12">
                            {contactMethods.map((method, index) => (
                                ...
                                >
                                    ...
                                    {method.action ? (
                                        <a href={method.action} className="text-[#137fec] hover:underline font-bold transition-colors">
                                            {method.value}
                                        </a>
                                    ) : (
                                        <span className="text-slate-900 dark:text-white font-bold">{method.value}</span>
                                    )}
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="grid lg:grid-cols-2 gap-12">
                            ...
                            {/* FAQ Section */}
                            <div>
                                <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-6">Frequently Asked</h2>
                                    ...
                                    {faqs.map((faq, index) => (
                                        ...
                                        >
                                            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2 flex items-start gap-3">
                                                <HelpOutlineIcon className="text-[#137fec] mt-0.5" fontSize="small" />
                                                {faq.question}
                                            </h3>
                                            <p className="text-slate-600 dark:text-slate-400 leading-relaxed pl-9 text-sm">{faq.answer}</p>
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className="mt-8 p-6 bg-blue-50 dark:bg-slate-800/50 rounded-2xl border border-blue-100 dark:border-slate-700">
                                    <div className="flex items-start gap-4">
                                        <LightbulbIcon className="text-[#137fec]" fontSize="large" />
                                        <div>
                                            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">Looking for more answers?</h3>
