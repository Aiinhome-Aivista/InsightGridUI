...
export default function Sidebar() {
  ...
  h-full border-r shrink-0 transition-all duration-200 flex flex-col`}
    ...
    >
        ...
        >
          <div className="flex items-center">
            <AccountCircleRoundedIcon
              sx={{ color: theme.secondaryText, fontSize: "2rem" }}
            />
            {!collapsed && (
              <p
        ...
        >
          {collapsed ? (
            <ArrowForwardIosRoundedIcon
              sx={{ fontSize: "small", color: theme.secondaryText }}
            />
          ) : (
        ...
        <nav className={`flex flex-col gap-2 ${collapsed ? "px-4" : "px-2"}`}>
          {menuItems
            ...
            .map((item) => {
              ...
              if (
                item.path === "query-list" &&
                activePath === "query-designer"
              ) {
                isActive = true;
              }
              if (
                item.path === "report-designer" &&
                activePath === "report-designer-view"
              ) {
                isActive = true;
              }
              if (
                item.path === "manage-company-users" &&
                activePath === "add-company-user"
              ) {
                isActive = true;
              }
              if (
                item.path === "manage-companies" &&
                location.pathname.includes("register-company")
              ) {
                isActive = true;
              }
              if (
                item.path === "manage-company-user" &&
                location.pathname.includes("add-company-user")
              ) {
                isActive = true;
              }
              if (
                item.path === "manage-seo" &&
                location.pathname.includes("add-seo")
              ) {
                isActive = true;
              }
              if (
                item.path === "report-scheduler" &&
                activePath === "add-report-scheduler"
              ) {
                isActive = true;
              }
              return (
      ...
      <div
        className={`mt-auto py-4 ${collapsed ? "px-4" : "px-3"}`}
        style={{ borderColor: theme.border }}
      >
        <Tippy
          content="Logout"
